            ...
            if command == 'run':
                ...
                for e in range(int(args)):
                    ...
                    while True:
                        ...
                        # 选择动作：只从 valid_actions 中按概率选
                        action = np.random.choice(len(policy), p = policy)
                        next_state, reward, done = env.step(action)  # apply the action to env
                        if done:
                            train_examples = [
                                (env.compute_canonical_form_obs(x[0], x[2]), x[1],
                                 reward * (-1 if x[2] != player else 1))
                                for x in train_examples
                            ]
                            all_examples += train_examples  # record all data
...
class AlphaZeroParallel:
    ...
    def evaluate(self):
        ...
        self.win_rates.append(result[0][0] / (result[0][0] + result[0][1] + result[0][2]))
        self.unbeaten_rates.append((result[0][0] + result[0][2]) / (result[0][0] + result[0][1] + result[0][2]))
    ...
    def learn(self):
        ...
        for iter in range(1, self.config.n_train_iter + 1):
            ...
            if win_rate > self.config.update_threshold:
                ...
                self.win_rates.append(self.win_rates[-1])
                self.unbeaten_rates.append(self.unbeaten_rates[-1])
        ...
        self.plot_win_rate_and_draw_rate(self.win_rates, self.unbeaten_rates)

    def close(self):
        for worker in self.workers:
            worker.close()

    def plot_win_rate_and_draw_rate(self, win_rates, draw_rates):
        """绘制胜率和不败率的折线图"""
        plt.figure(figsize=(10, 6))
        plt.plot(range(1, len(win_rates) + 1), win_rates, label="Win Rate", color="b", marker="o")
        plt.plot(range(1, len(draw_rates) + 1), draw_rates, label="Unbeaten Rate", color="g", marker="x")
        plt.xlabel("Iteration")
        plt.ylabel("Rate")
        plt.title("Win Rate and Unbeaten Rate Over Training Iterations")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("win_rate_and_unbeaten_rate.png")  # Save the plot to a file
