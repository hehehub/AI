...
class NumpyLinearModel():
    def __init__ (
        ...
        if str(device) != "cpu":
            print(
                f"[WARNING] Linear model is designed to be runing on CPU only. The device will be set to CPU instead of {device}")
        self.action_size = action_space_size
        self.observation_size = reduce(lambda x, y: x * y, observation_size, 1) if isinstance(observation_size,
                                                                                              Iterable) else observation_size
    ...
    def grad_v (self, X: np.ndarray, v_trgt: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        ########################
        B = X.shape[0]
        z = X @ self.w_v.reshape(-1, 1) + self.b_v
        v = np.tanh(z)
        delta = (v - v_trgt) * (1 - v ** 2)
        grad_w_v = (2 * X.T @ delta).reshape(-1) / B  # shape (O, )
        grad_b_v = 2 * np.mean(delta, axis = 0)  # shape (1,)
...
def check_grad (model: NumpyLinearModel):
    ...
    print(
        f"[Gradient of w_v] shape:{error_grad_w_v.shape}   \tMean Error: {np.mean(error_grad_w_v):.6f}, Max Error: {np.max(error_grad_w_v):.6f}")
    print(
        f"[Gradient of b_v] shape:{error_grad_b_v.shape}        \tMean Error: {np.mean(error_grad_b_v):.6f}, Max Error: {np.max(error_grad_b_v):.6f}")
    ...
    print(
        f"[Gradient of w_pi] shape:{error_grad_w_pi.shape}   \tMean Error: {np.mean(error_grad_w_pi):.6f}, Max Error: {np.max(error_grad_w_pi):.6f}")
    print(
        f"[Gradient of b_pi] shape:{error_grad_b_pi.shape}   \tMean Error: {np.mean(error_grad_b_pi):.6f}, Max Error: {np.max(error_grad_b_pi):.6f}")
